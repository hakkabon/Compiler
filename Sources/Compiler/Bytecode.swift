...
public enum BytecodeValidator {
    public static func validate(_ code: [Instruction], localCount: Int) throws {
        for (index, instruction) in code.enumerated() {
            ...
            guard instruction.operands.count == expected else {
                throw Diagnostic(stage: .validation, message: "\(instruction.opcode) at \(index) expects \(expected) operand(s)")
            }
        ...
        while let index = worklist.popLast() {
            ...
            let effect = stackEffect(code[index].opcode)
    ...
    public static func validate(_ code: [Instruction3], localCount: Int, registerCount: Int = 32) throws {
        for (index, instruction) in code.enumerated() {
            ...
            case .jumpIfFalse:
                guard case .register = instruction.source1 else {
            ...
            case .loadLocal:
                ...
                guard case .register = instruction.destination else { throw shape(index, instruction.opcode) }
            case .storeLocal:
            ...
            case .move:
                guard case .register = instruction.destination, instruction.source1 != nil else {
            ...
            case .add, .sub, .mul, .div, .mod, .eq, .ne, .lt, .le, .gt, .ge:
                guard case .register = instruction.destination, instruction.source1 != nil, instruction.source2 != nil else {
                    throw shape(index, instruction.opcode)
                }
            case .print:
                guard instruction.source1 != nil else { throw shape(index, instruction.opcode) }
    ...
    private static func stackOperandCount(_ opcode: StackOpcode) -> Int {
        [.push, .loadLocal, .storeLocal, .jump, .jumpIfFalse].contains(opcode) ? 1 : 0
    }
    private static func stackEffect(_ opcode: StackOpcode) -> (minimum: Int, delta: Int) {
        switch opcode {
